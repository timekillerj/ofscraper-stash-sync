...
class MediaHandler:
    ...
    def parse_text_for_performers(self, text):
        ...
        # This regex allows for html tags before name (<a href=''>@name</a>),
        # periods and dashed in name, and ignores punctuation after name
        # just the name without the '@' is returned
        mentions = re.findall(
            r"(?:^|\s|>)@([\w\-]+(?:\.[\w\-]+)*)(?=[\s\.\?\!\u2026<,:;]|$)",
            text
        )
